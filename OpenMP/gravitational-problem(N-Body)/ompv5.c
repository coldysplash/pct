...
void move_particles(struct particle *p, struct particle *f[],
...
int main(int argc, char *argv[]) {
  ...
  struct particle *f[omp_get_max_threads()];  // Сила, действующая на каждую
                                              // частицу (x, y, z)
...
#pragma omp parallel num_threads( \
    nthrds)  // Параллельный регион активируется один раз
  {
...
#pragma omp barrier  // Ожидание завершения обновления p[i], f[i]
  ...
  printf("# NBody (n=%d)\n", n);
  printf(
      "# Elapsed time (sec): ttotal %.6f, tinit %.6f, tforces %.6f, tmove "
      "%.6f\n",
  ...
  free(m);
  free(v);
  for (int i = 0; i < omp_get_max_threads(); i++) free(f[i]);
  free(p);
  return 0;
}
